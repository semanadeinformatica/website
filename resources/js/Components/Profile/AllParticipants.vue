...
const allParticipants = computed(
    () => (page.props.allParticipants as Visitor[]) ?? [],
);
...
const cvCount = computed(
    () => allParticipants.value.filter((v) => v.can_see_cv && v.cv_url).length,
);
...
const filterOptions = computed<PillOption[]>(() => [
    {
        id: "all",
        label: "Todos",
        count: allParticipants.value.length,
    },
    {
        id: "linkedin",
        label: "Com LinkedIn",
...
const filteredParticipants = computed(() => {
    return allParticipants.value.filter((v) => {
...
<template>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-neutral-300">
                    <tbody class="divide-y divide-white/6">
                        <tr
                            v-for="visitor in filteredParticipants"
                            :key="visitor.id"
    ...
    >
        ...
        <h3 class="text-base font-semibold text-white">
            Ainda sem participantes registados
        </h3>
        <p class="mt-1 max-w-sm text-xs text-neutral-400 sm:text-sm">
            Os participantes inscritos na edição aparecerão aqui.
        </p>
    </div>
</template>
