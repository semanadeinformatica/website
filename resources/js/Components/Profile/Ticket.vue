...
import { Calendar, Clock, MapPin } from "@lucide/vue";
...
const mainColor = () => {
    if (props.event.capacity && props.event.capacity > 0) {
        return "bg-sinf-primary-dark";
    } else {
        return "bg-sinf-secondary-dark";
    }
};

const accentColor = () => {
    if (props.event.capacity && props.event.capacity > 0) {
        return "bg-sinf-primary";
    } else {
        return "bg-sinf-secondary";
    }
};
</script>
...
<template>
    <Link
        ...
        class="group relative block w-87.5 max-w-full justify-self-center transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]"
    >
        <div
            class="@container grid aspect-5/2 w-full place-content-stretch place-items-stretch drop-shadow-[0_4px_16px_rgba(0,0,0,0.5)]"
            :class="[
                state == 'available'
                    ? 'cursor-pointer opacity-60 grayscale transition-all select-none hover:opacity-100 hover:grayscale-0'
                    : '',
            ]"
        >
            <svg
                class="col-start-1 row-start-1 h-full w-full overflow-visible"
                viewBox="0 0 500 200"
            >
                <path
                    v-if="state != 'used'"
                ...
                <template v-else>
                    <path
                        class="origin-left -translate-x-1.5 -rotate-6"
            ...
            <div
                class="col-start-1 row-start-1 flex h-full w-full text-[3.2cqi] font-black text-white"
            ...
            >
                ...
                >
                    <span
                        ...
                        :class="accentColor()"
                    >
                        NR: {{ hashCode((event.id + "SINF2026").toString()) }}
                    </span>

                    <div
                        class="my-auto flex min-w-0 flex-col gap-[.25em] px-[.75em] py-[.5em]"
                    >
                        <div
                            v-if="event.location"
                            :title="event.location"
                            class="flex min-w-0 items-center gap-[.35em]"
                        >
                            <MapPin
                                :size="12"
                        ...
                        <div
                            v-if="event.event_day?.date"
                            :title="$d(event.event_day?.date, 'short')"
                            class="flex min-w-0 items-center gap-[.35em]"
                        >
                            <Calendar
                                :size="12"
